...
vec4 iso_dim_light(vec4 position, vec4 omega, vec4 normal){
    return vec4(1.0f, 1.0f, 1.0f, 10.0f);
}

vec4 iso_bright_light(vec4 position, vec4 omega, vec4 normal){
...
vec4 iso_blue(vec4 position, vec4 omega, vec4 normal){
    return vec4(0.0f, 0.0f, 10.0f, 10.0f);
}

vec4 iso_green(vec4 position, vec4 omega, vec4 normal){
    return vec4(0.0f, 1.0f, 0.0f, 10.0f);
}

vec4 iso_red(vec4 position, vec4 omega, vec4 normal){
    return vec4(1.0f, 0.0f, 0.0f, 10.0f);
}
...
int main(){
    ...
    plane2->material->diffuse = iso_green;
    ...
    // plane3->material->emmission = iso_dim_light;
    ...
    sphere4->material->emmission = iso_bright_light;
    ...
    Ray_Tracer r;
    if (!r.initialize("Example 1", 640, 480, 2, 1, 30, "ray_trace")){
